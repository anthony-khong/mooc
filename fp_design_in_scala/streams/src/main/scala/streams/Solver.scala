...
trait Solver extends GameDef {
   ...
   */
  def done(block: Block): Boolean = block.isStanding && (block.b1 == goal)
  ...
  def neighborsWithHistory(block: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    for ((newBlock, move) <- block.legalNeighbors.toStream) yield (newBlock, move :: history)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, history) #:: tail => {
      val neighbours = neighborsWithHistory(block, history)
      val newNeighbours = newNeighborsOnly(neighbours, explored)
      (block, history) #:: from(tail #::: newNeighbours , explored + block)
    }
  }
